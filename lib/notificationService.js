...
class NotificationService {
  ...
  async sendEmailVerification(email, verificationToken, user = {}) {
    ...
    if (!this.emailConfig.serviceId || this.emailConfig.serviceId === 'your_emailjs_service_id') {
      // Console fallback for development
      console.warn('📧 EmailJS not configured - Email verification requested for:', email);
      console.warn('✅ Verification URL:', verificationUrl);
      console.warn('📝 Configure EmailJS in .env.local to send real emails');
      ...
      return {
        ...
        method: 'console',
        contact: email,
        message: 'Verification link logged to console (configure EmailJS for real emails)',
        verificationUrl: verificationUrl,
        instructions: 'Check server console for verification link'
      };
    }

    if (!this.emailConfig.templates.emailVerification) {
      return {
        success: false,
        error: 'Email verification template not configured',
        service: 'emailjs'
      };
    }
    ...
    try {
      ...
      const response = await emailjs.send(
        ...
        this.emailConfig.templates.emailVerification,
        templateParams
      );
      ...
      console.error('EmailJS email verification error:', error);
      return {
        success: false,
        error: error.text || error.message || 'Failed to send email verification',
        service: 'emailjs',
        method: 'email'
      };
    }
  ...
  async _sendEmailPasswordReset(email, resetUrl, user) {
    ...
    if (!this.emailConfig.serviceId || this.emailConfig.serviceId === 'your_emailjs_service_id') {
      // Console fallback for development
      console.warn('📧 EmailJS not configured - Password reset requested for:', email);
      ...
      return {
        ...
        method: 'console',
        contact: email,
        message: 'Reset link logged to console (configure EmailJS for real emails)',
        resetUrl: resetUrl,
    ...
    try {
      ...
      console.log('📧 Sending real password reset email via EmailJS...', {
        service: this.emailConfig.serviceId,
